...
public class InterviewSetupService {
    ...
    public InterviewSessionResponse createSession(InterviewSetupRequest request) {
        ...
        List<Competency> competencies = request.competencyNames().stream()
                .map(name -> competencyRepository.findByName(name)
                        .orElseThrow(() -> new IllegalArgumentException("Unknown competency: " + name)))
                .toList();
        ...
        for (Competency competency : competencies) {
            List<Question> picked = questionRepository.findRandomByCompetency(
                    competency.getId(), request.questionsPerCompetency());
        ...
        var summaries = sessionQuestions.stream()
                .map(sq -> new InterviewSessionResponse.QuestionSummary(
                        sq.getId(), sq.getQuestion().getText(), sq.getSequenceNo()))
                .toList();

        return new InterviewSessionResponse(
                session.getId(), session.getRole(),
                session.getSeniority().name(), session.getPersonaStyle().name(),
                summaries
        );
    }
}